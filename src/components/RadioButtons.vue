<template>
  <div class="background">
    <h1 class="nameQ">What kind of website would you like?</h1>
    <br />
    <div class="radios">
      <input type="radio" value="blog" id="blog" />
      <label class="radio" for="blog"> Blog</label>
      <br />

      <input type="radio" value="portfolio" id="portfolio" />
      <label class="radio" for="portfolio"> Portfolio</label>
      <br />

      <input type="radio" value="restaurant" id="restaurant" />
      <label class="radio" for="restaurant"> Restaurant</label>
      <br />

      <input type="radio" value="event" id="event" />
      <label class="radio" for="event"> Event</label>

      <input type="radio" value="e-commerce" id="e-commerce" />
      <label class="radio" for="e-commerce"> E-commerce</label>
      <br />
    </div>
    <div class="buttons">
      <button class="backButton">Back</button>
      <button class="nextButton">Next</button>
    </div>
  </div>
</template>
